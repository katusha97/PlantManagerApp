...
class DatabaseHandlerImpl(private val context: Context) : DatabaseHandler {
    ...
    override fun addRoom(left_up: Double, right_down: Double) {
        db.execSQL("INSERT INTO rooms(\"left_up\", \"right_down\") VALUES (${left_up}, ${right_down})")
    }
    ...
    override fun addPlant(plant_id: Int, name: String, room_id: Int, watering_day: String,
        ...
        db.execSQL("INSERT INTO plants_of_user VALUES (${plant_id}, ${name}, ${room_id}, ${watering_day}, " +
                "${transplant_day}, ${fertilization_day}, ${period_id}, ${coord_x}, ${coord_y})")
    }

    override fun removeRoom(room_id: Int) {
        db.delete("rooms", "room_id = $room_id", null)
    }

    override fun removePlant(name: String) {
        db.delete("plants_of_user", "name = $name", null)
    }

    override fun removeWork(date: String, work_id: Int, plant_id: Int) {
        db.delete("calendar", "date = $date and work_id = $work_id and plant_id = $plant_id", null)
    }
    ...
    override fun setNotifTime(notif_time: String, every_X_minute: Int) {
        ...
        db.update("user_app", values, null, null)
    }
    ...
    override fun setNewUser(name: String, email: String, birthday: String) {
        ...
        db.insert("user_app", null, values)
    }
    ...
    override fun setPeriodOfLife(name: String, period_id: Int) {
        ...
        db.update("plants_of_user", values, "name = $name", null)
    }
    ...
    override fun updateDateOfWork(date: String, plant_id: Int, work_id: Int, new_date: String) {
        ...
        db.update("calendar", values, "day = $date and " +
    ...
    override fun getPlant(name: String): Plant {
        val cursor = db.rawQuery("SELECT * FROM plants_of_user WHERE name = $name", null)
    ...
    override fun getAllPlantsOfUser(): List<Plant> {
        val cursor = db.rawQuery("SELECT * FROM plants_of_user", null)
    ...
    override fun getAllPlantByRoom(room_id: Int): List<Plant> {
        val cursor = db.rawQuery("SELECT * FROM plants_of_user WHERE room_id = $room_id", null)
        val listOfPlants = ArrayList<Plant>()
        cursor.moveToFirst()
        while (cursor.isAfterLast) {
            listOfPlants.add(Plant(cursor.getString(cursor.getColumnIndex("plant_id")),
    ...
    override fun getDescription(name: String): String {
        ...
        val cursor = db.rawQuery("SELECT description FROM plants WHERE id = $id", null)
        return cursor.getString(cursor.getColumnIndex("description"))
    }

    @SuppressLint("Recycle")
    override fun getNotifTime(): String {
        val cursor = db.rawQuery("SELECT notification_time FROM user_app", null)
        return cursor.getString(cursor.getColumnIndex("notification_time"))
    }

    @SuppressLint("Recycle")
    override fun getLanguage(): String {
        val cursor1 = db.rawQuery("SELECT language_id FROM user_app", null)
        val cursor = db.rawQuery("SELECT name FROM language WHERE id = " +
    ...
    override fun getAllWorkForDay(date: String): List<Work> {
        val cursor = db.rawQuery("SELECT * FROM calendar WHERE day = $date", null)
    ...
    override fun getPeriodOfLife(name: String): String {
        val cursor1 = db.rawQuery("SELECT period_id FROM plants_of_user WHERE name = $name", null)
        val cursor = db.rawQuery("SELECT name from period_of_life WHERE id = " +
    ...
    override fun getPeriodId(name: String): Int {
        val cursor = db.rawQuery("SELECT id from period_of_life WHERE name = $name", null)
        return cursor.getColumnIndex("id")
    }

    @SuppressLint("Recycle")
    override fun getPlantId(name: String): Int {
        val cursor = db.rawQuery("SELECT id from plants_of_user WHERE name = $name", null)
        return cursor.getColumnIndex("id")
    }

    @SuppressLint("Recycle")
    override fun getWorkId(work: String): Int {
        val cursor = db.rawQuery("SELECT id from work_enum WHERE work = $work", null)
        return cursor.getColumnIndex("id")
    }

    @SuppressLint("Recycle")
    override fun getLanguageId(language_name: String): Int {
        val cursor = db.rawQuery("SELECT id from language WHERE name = $language_name", null)
        return cursor.getColumnIndex("id")
    }
}
